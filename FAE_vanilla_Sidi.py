...
eval_mse_sdse(input, FAE_pred)

FAE_pred_train, reps, pred_loss, pred_score_loss = pred(model, TrainData)
eval_mse_sdse(TrainData, FAE_pred_train)
...
eval_mse_sdse(TrainData, FAE_pred_train)
eval_mse_sdse(TrainData, FPCA_pred_train)
...
plt.ioff()
pdf = matplotlib.backends.backend_pdf.PdfPages("Datasets/ElNino/ElNino_2layers(20-10-5)_nonlinear(SiLU)_0.2Test2.pdf")
...
for i in range(len(input_plt)): ## will open an empty extra figure :(\
    ...
    plt.plot(tpts, AE_pred_plt[i], label="AE-pred")
    plt.legend()
